...
TcpSocket::TcpSocket(std::string ip, uint16_t port) : m_addr(ip, port) {
    ...
    ::setsockopt(m_socket_fd, SOL_SOCKET, SO_REUSEADDR, &optval,
                 sizeof(optval));
    if (m_socket_fd < 0) {
        spdlog::error("Failed to create socket!");
        abort();
    }
    if (::bind(m_socket_fd, (struct sockaddr *)&m_addr.get_addr(),
               sizeof(m_addr.get_addr())) < 0) {
...
std::pair<bool, TcpSocket::socket_ptr> TcpSocket::accept() {
    ...
    int conn_fd = ::accept4(m_socket_fd, (sockaddr *)&client_addr, &addr_len,
                            SOCK_NONBLOCK);
    auto conn_sock =
        std::make_shared<TcpSocket>(conn_fd, InetAddr(std::move(client_addr)));

    if (conn_fd == -1)
        return {false, conn_sock};
    else
        return {true, conn_sock};
}
...
void TcpSocket::ignore_sigpipe() { ::signal(SIGPIPE, SIG_IGN); }
...
int TcpSocket::send(std::string &buf) {
    ...
    while (already_send_bytes < to_send_bytes) {
        send_bytes = ::send(m_socket_fd, ptr + already_send_bytes,
                            to_send_bytes - already_send_bytes, 0);
        if (send_bytes <= 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK)
                break;
            if (errno == EINTR)
