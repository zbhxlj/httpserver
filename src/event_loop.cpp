...
EventLoop::EventLoop(int timeout)
    : m_is_looping(false), m_is_quit(false), m_pid(tl_pid),
      m_poller(std::make_unique<Epoll>()), m_wakeup_fd(create_event_fd()),
      m_wakeup_channel(std::make_shared<Channel>(
          std::make_shared<TcpSocket>(m_wakeup_fd, InetAddr()), this)),
    ...
    tl_event_loop = this;
    m_wakeup_channel->set_read_cb(std::bind(&EventLoop::clean_wakeup, this));
...
EventLoop *EventLoop::get_eventloop_of_cur_thread() { return tl_event_loop; }
void EventLoop::loop() {
    assert(!m_is_looping);
    ...
    while (!m_is_quit) {
        ...
        for (auto &channel : active_channels) {
            spdlog::debug(
                "handle channel socket_fd = {}, triggered events = {}",
                channel->get_fd(), channel->get_triggered_events());
            // process IO event
...
void EventLoop::flush_keep_alive(channel_ptr &channel,
