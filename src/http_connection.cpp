...
HttpConnection::HttpConnection(EventLoop *loop, socket_ptr conn_fd)
    : m_loop(loop), m_conn_fd(conn_fd),
      m_channel(std::make_shared<Channel>(conn_fd, loop)), m_handler(),
      m_state(Connected) {
...
void HttpConnection::handle_read() {
    ...
    // spdlog::debug("nbytes = {}, is_finished = {}, buf = {}", nbytes,
    // is_finished, m_in_buffer); error management
    if (nbytes < 0) {
...
void HttpConnection::shut_down(int how) { m_conn_fd->shutdown(how); }
} // namespace webserver