...
void HttpHandler::set_url(const std::string &url) { m_url = url; }
...
HttpHandler::HttpHandler(EventLoop *loop, socket_ptr conn_fd)
    : m_loop(loop), m_conn_fd(conn_fd),
      m_connection(std::make_unique<HttpConnection>(loop, conn_fd)),
...
void HttpHandler::handle_keep_alive() {
     ...
     */
    HttpConnection::ConnState connState = m_connection->get_state();
    if (connState == HttpConnection::DisConnecting)
        return;
